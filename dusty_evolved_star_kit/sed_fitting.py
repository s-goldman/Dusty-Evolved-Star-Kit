...
from desk.python_scripts import config, remove_old_files, plotting_seds, get_padova
from desk.python_scripts.plotting_seds import create_fig
...
def main():
    ...
    for item in os.listdir('desk/put_target_data_here/'):
        if fnmatch(item, "*csv"):
            targets.append('desk/put_target_data_here/' + item)
    # example source
    ...
    # check if padova
    if not os.path.isfile('desk/models/' + config.fitting['model_grid'] + '_models.fits'):
    ...
    grid_dusty = Table.read('desk/models/' + config.fitting['model_grid'] + '_models.fits')
    grid_outputs = Table.read('desk/models/' + config.fitting['model_grid'] + '_outputs.csv')
    ...
    file_b.write('desk/fitting_plotting_outputs.csv', format='csv', overwrite=True)
